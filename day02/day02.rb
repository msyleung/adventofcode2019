class Intcode
  ...
  def brute_force_calc_gravity_assist(goal = 19_690_720)
    new_verb = -1
    while new_verb <= 100 && (addr0 = @memory[0]) != goal
      ...
      while new_noun <= 100
        ...
        puts "at #{addr0} : #{addr0 - 19_690_720} from goal: 19690720"
        new_noun += 1
  ...
  # want diff = 0
  def faster_calc_gravity_assist(goal = 19_690_720, new_noun = 50, new_verb = 50)
    set_program_alarm(new_noun, new_verb)
    intcode_reader
    return new_noun, new_verb if (addr0 = @memory[0]) == goal

    restore_memory
    # puts "diff: #{addr0 - goal} @ #{new_noun},  #{new_verb}"
    if (diff = addr0 - goal) < -100 && (new_noun + 1) <= 99
      faster_calc_gravity_assist(goal, new_noun + 1, new_verb)
    elsif diff > 100 && (new_noun - 1) >= 0
      faster_calc_gravity_assist(goal, new_noun - 1, new_verb)
    elsif diff > 100 && (new_verb + 1) <= 99
      faster_calc_gravity_assist(goal, new_noun, new_verb + 1)
    elsif diff <= 100 && (new_verb - 1) >= 0
      faster_calc_gravity_assist(goal, new_noun, new_verb - 1)
    end
  end

  # TODO: method has too many lines: 13/10
  # The 3 integers immediately after the opcode tell you these 3 positions
  # the first 2 indicate the positions from which you should read the inputs
  # and the 3rd indicates the position at which the output should be stored.
  ...
  def intcode_reader
    ...
    @memory.each.with_index do |opcode, instruction_pointer|
      next_instruction.zero? ? next_instruction : next_instruction -= 1
